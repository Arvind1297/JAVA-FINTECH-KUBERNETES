...
TARGET_URL = "http://api.bank.mymorena.xyz/api/auth/register"
TOTAL_REQUESTS = 5000
CONCURRENCY = 100
...
async def run_load_test(total: int, concurrency: int) -> dict:
    ...
    return {
        ...
        "p95_response_time_ms": round(
            percentile(response_times, 0.95), 1
        ),
        "p99_response_time_ms": round(
            percentile(response_times, 0.99), 1
        ),
...
async def main():
    ...
    print(
        f"Firing {TOTAL_REQUESTS} requests "
    ...
    print(
        f"p95 response time: "
        f"{result['p95_response_time_ms']} ms"
    )

    print(
        f"p99 response time: "
        f"{result['p99_response_time_ms']} ms"
    )
